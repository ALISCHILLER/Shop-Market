...
SELECT
	...
	CASE
    	WHEN CustomerCallReturnLinesView.RequestBulkUnitId IS NULL THEN
    		sum( CustomerCallReturnLinesView.TotalReturnQty ) * ( IFNULL( Product.productWeight, 0 ) * 0.001 )
    	ELSE sum( CustomerCallReturnLinesView.TotalReturnQty )
    END AS TotalWeight
FROM
...
AND CustomerOldInvoiceDetail.SaleId = CustomerCallReturnLinesView.InvoiceId
LEFT JOIN Product ON CustomerCallReturnLinesView.ProductId = Product.UniqueId
GROUP BY
