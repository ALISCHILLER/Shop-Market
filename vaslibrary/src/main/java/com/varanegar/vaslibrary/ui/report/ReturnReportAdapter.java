...
public class ReturnReportAdapter extends SimpleReportAdapter<CustomerCallReturnViewModel> {
    private final boolean withRef;
    private boolean returnTypeEnabled;

    public ReturnReportAdapter(MainVaranegarActivity activity, boolean withRef) {
        super(activity, CustomerCallReturnViewModel.class);
        this.withRef = withRef;
    }

    @Override
    protected ItemContextView<CustomerCallReturnViewModel> onCreateContextView() {
        return new ReturnReportAdapterContextView(getAdapter(), getActivity());
    }

    @Override
    public void bind(ReportColumns columns, CustomerCallReturnViewModel entity) {
        bindRowNumber(columns);
        columns.add(bind(entity, CustomerCallReturnView.ProductCode, activity.getString(R.string.product_code)).setSortable().setFrizzed());
        columns.add(bind(entity, CustomerCallReturnView.ProductName, activity.getString(R.string.product_name)).setSortable().setWeight(2));
        columns.add(bind(entity, CustomerCallReturnView.TotalReturnQty, activity.getString(R.string.returned_qty_label)).setCustomViewHolder(new CustomViewHolder<CustomerCallReturnViewModel>() {
            @Override
            public void onBind(View view, CustomerCallReturnViewModel entity) {
        ...
        columns.add(bind(entity, CustomerCallReturnView.TotalReturnQty, activity.getString(R.string.total_qty)).setSortable().setWeight(1).calcTotal());
        if (withRef) {
            columns.add(bind(entity, CustomerCallReturnView.IsPromoLine, activity.getString(R.string.prize)).setCustomViewHolder(new CustomViewHolder<CustomerCallReturnViewModel>() {
        ...
        columns.add(bind(entity, CustomerCallReturnView.TotalRequestAmount, activity.getString(R.string.return_amount)).setSortable().setWeight(1).calcTotal());

        if (withRef) {
            columns.add(bind(entity, CustomerCallReturnView.TotalRequestNetAmount, activity.getString(R.string.sell_return_net_amount)).setSortable().setWeight(1.5f).calcTotal());
            columns.add(bind(entity, CustomerCallReturnView.SaleNo, activity.getString(R.string.invoice_no)).setSortable());

            columns.add(bind(entity, CustomerCallReturnView.InvoiceQty, activity.getString(R.string.invoice_qty_label)));
        }
    }

    class ReturnReportAdapterContextView extends ItemContextView<CustomerCallReturnViewModel> {

        private ArrayList<ReturnCalculatorItem> returnCalculatorItemList = new ArrayList<>();

        public ReturnReportAdapterContextView(BaseRecyclerAdapter<CustomerCallReturnViewModel> adapter, Context context) {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup viewGroup, int position) {
            ...
            CustomerCallReturnViewModel returnViewModel = adapter.get(position);
