...
public class CompleteReturnActionDialog extends CuteAlertDialog {
    ...
    protected void onCreateContentView(LayoutInflater inflater, ViewGroup viewGroup, @Nullable Bundle savedInstanceState) {
        setTitle(R.string.please_select_a_reason_of_complete_return);
    ...
    public void ok() {
        ...
        if (noSaleReasonModel == null) {
            Toast.makeText(getContext(), R.string.please_select_a_reason_of_complete_return, Toast.LENGTH_SHORT).show();
            return;
        }
        getDialog().dismiss();
        ...
        try {
            ...
            customerCallManager.saveCompleteReturn(customerId, invoiceIds, noSaleReasonModel.UniqueId);
            Timber.i("successfully update customer status to complete return");
