...
public class PromotionApplyStatuteOnEVCItemPreviewSDSV3 {
	...
	public static DiscountPrizeViewModel applyStatpromouteOnEVCItemPreview(String evcId, String callId, int discountId, int saleId, int prizeType, int evcType, int orderId, int dcRef)
	{
		int calcprizeType = 0;
		...
		if(fillEVCAcceptedDiscountTemp(evcId, discountId) == 0) return null;
		int prizePackageRef = EVCTempGoodsPackageItemSDSDBAdapter.getInstance().getPrizePackageRef(discountId);
		//insert into #GoodsPackageItem
		DiscountPrizeViewModel havetoChange = EVCTempGoodsPackageItemSDSDBAdapter.getInstance().fillEVCGoodPackageTemp(evcId, discountId, prizePackageRef, saleId, prizeType, evcType, orderId);
		if(fillPrizeItemsInEVCItem(evcId, callId, discountId, calcprizeType, evcType, saleId ,dcRef) == 0) return null;

		if(calcprizeType == 1){
			EVCItemSDSDBAdapter.getInstance().updatePrice(discountId);
		}

		if(ApplyStatuteOnEVCItem_CheckGoodsNoSale(evcId) != 0) return null;
		fillEVCPrizePackageOrEVCPrize(evcId, discountId, prizeType, prizePackageRef, evcType, orderId);
		deleteInvalidPrize(evcId);
		mergePrize(evcId, evcType);
		return havetoChange;
	}
/*
	public static void applyStatpromouteOnEVCItemPreview(String evcId, String callId, int discountId, int saleId, int prizeType, int evcType, int orderId, int dcRef)
	{
		int calcprizeType = 0;
		if (evcType == 2 || evcType == 10 ||evcType == 12 )
			calcprizeType = DiscountCustomerOldInvoiceDetailDBAdapter.getInstance().getPrizeCalcType(saleId);
...
*/
	private static void fillEVCPrizePackageOrEVCPrize(String evcId, int discountId, int prizeType, int prizePackageRef, int evcType, int orderId){
			String sql;
		if (prizePackageRef != 0)
			DiscountEvcPrizePackageDBAdapter.getInstance().insertByPrizePackageRef(evcId, discountId, prizeType, prizePackageRef);
		else
			if (prizeType == 6) {
				DiscountEvcPrizeDBAdapter.getInstance().insertByGoodPackage(evcId, discountId, prizeType, prizePackageRef, evcType, orderId);
			}
			else {
				DiscountEvcPrizePackageDBAdapter.getInstance().insertByGoodPackage(evcId, discountId, prizeType, prizePackageRef);
			}
	}
...
//                            LEFT OUTER JOIN DiscountProductUnitDBAdapter.DATABASE_TABLE P ON P.GoodsRef= D.PrizeRef AND P.UnitRef= D.PrizeUnit
//                            LEFT OUTER JOIN DiscountProductUnitDBAdapter.DATABASE_TABLE P3 ON P3.GoodsRef= D.GoodsRef AND P3.UnitRef= D.QtyUnit
//
...
//                                    INNER JOIN DiscountProductUnitDBAdapter.DATABASE_TABLE P2
...
//        end
	...
	/*Sle.usp_ApplyStatuteOnEVCItem_CheckGoodsNoSale*/
